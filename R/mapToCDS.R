...
setMethod(
  ...
  definition = function(
    ...
    for (chr in chrom) {
      ...
      vars_chr <- unserialize(getAnno(
        object,
        "var_ranges",
        where = sprintf("CHROM = '%s%s'", if (chrom_check) "chr" else "", chr)
      )$ranges[[1]])
        ...
        if (length(vars) > 0) {
          ...
          if (length(deductionsPlus) == 1) {
            ...
            deductionsMinus <- rev(c(
              0,
              cumsum(rev(width(GenomicRanges::gaps(exons_)[2:length(exons_)])))
            ))
          }
          ...
          if (length(overlaps_padded) > 0) {
            ...
            vars_padded$distStart <- start(exons_[S4Vectors::subjectHits(
              overlaps_padded
            )]) -
              end(vars_padded) -
              1
            vars_padded$distEnd <- start(vars_padded) -
              end(exons_[S4Vectors::subjectHits(overlaps_padded)]) -
              1
            vars_padded$POS_updated = ifelse(
              vars_padded$distStart >= 0 & vars_padded$distStart <= exonPadding,
            ...
            mcols(vars_padded) <- cbind(
              mcols(vars_padded),
              mcols(exons_[S4Vectors::subjectHits(overlaps_padded)])[, c(
                "gene_id",
                "transcript_id",
                "deductionsPlus",
                "deductionsMinus"
          ...
          if (length(overlaps) > 0) {
            ...
            mcols(vars) <- cbind(
              mcols(vars),
              mcols(exons_[S4Vectors::subjectHits(overlaps)])[, c(
                "gene_id",
                "transcript_id",
                "deductionsPlus",
                "deductionsMinus"
          ...
          if (length(vars) > 0) {
            if (strand == "+") {
              ...
              mcols(vars)[["cdsPOS"]] <- rep(
                max(end(exons_)) + 1,
                length(vars)
              ) -
                start(vars) -
                vars$deductionsMinus
            }
            names(vars) <- NULL
            vars <- as.data.frame(vars)[, c(
              "VAR_id",
              "seqnames",
              "start",
              "gene_id",
