...
Provide a valid function call for detailed help.\n%s",
      paste(
        rvat_cli_methods_flags[rvat_cli_methods_flags %in% args_raw],
        collapse = ","
      ),
...
Provide a valid function call for detailed help.\n%s",
  ...
  if ("buildVarSet" %in% args_raw) {
    ...
    expected <- c(
      ...
      "gdb",
      "quiet",
      names(.retrieve_formals_dispatch("buildVarSet", "gdb"))
    )
    check_args(
  ...
  if ("summariseGeno" %in% args_raw) {
    ...
    expected <- c(
      ...
      "quiet",
      "not-strict",
      names(.retrieve_formals_dispatch("summariseGeno", "gdb"))
    )
    check_args(
  ...
  if ("aggregate" %in% args_raw) {
    ...
    expected <- c(
      ...
      "quiet",
      "not-strict",
      names(.retrieve_formals_dispatch("aggregate", "gdb"))
    )
    check_args(
      func_name = "aggregate",
      args = args_raw,
  ...
  # mergeAggDbs
  if ("mergeAggDbs" %in% args_raw) {
    required <- c("filelist", "output")
    expected <- c(
      "mergeAggDbs",
      "quiet",
      "not-checkDups",
      names(formals(mergeAggDbs)),
      names(formals(aggdbList))
    )
    check_args(
    ...
    if (!args[["not-checkDups"]] || "checkDups" %in% args_raw) {
      checkDups <- TRUE
    } else {
      checkDups <- FALSE
    }
    aggdblist <- aggdbList(files, checkDups = checkDups)
  ...
  # collapseAggDbs
  if ("collapseAggDbs" %in% args_raw) {
    required <- c("filelist", "output")
    expected <- c(
      "collapseAggDbs",
      "quiet",
      "not-checkDups",
      names(formals(collapseAggDbs)),
      names(formals(aggdbList))
    )
    check_args(
  ...
  # assocTest-gdb
  if ("assocTest" %in% args_raw && !"aggdb" %in% args_raw) {
    ...
    expected <- c(
      ...
      "not-strict",
      "seed",
      names(.retrieve_formals_dispatch("assocTest", "gdb"))
    )
    check_args(
      func_name = "assocTest",
      args = args_raw,
  ...
  # assocTest-aggdb
  if ("assocTest" %in% args_raw && "aggdb" %in% args_raw) {
    required <- c("aggdb", "pheno", "test", "geneSet", "gdb", "output")
    expected <- c(
      "assocTest",
      "aggdb",
      "quiet",
      "not-strict",
      names(.retrieve_formals_dispatch("assocTest", "aggdb"))
    )
    check_args(
      func_name = "assocTest",
      args = args_raw,
    ...
    assocTest(
      ...
      continuous = args[["continuous"]],
      covar = covar,
      subtractCovar = args[["subtractCovar"]],
      dropUnits = dropUnits,
      maxitFirth = args[["maxitFirth"]],
      keep = keep,
      output = args[["output"]],
  ...
  if ("buildCorMatrix" %in% args_raw) {
    required <- c("buildCorMatrix", "rvbResult", "aggdb", "output")
    expected <- c(
      "buildCorMatrix",
      "rvbResult",
      "output",
