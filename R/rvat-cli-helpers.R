...
collect_args <- function() {
  parsed_opts <- optparse::OptionParser(
    option_list = rvat_cli_options,
    add_help_option = FALSE
  )
  args <- optparse::parse_args(parsed_opts, print_help_and_exit = FALSE)
  args_raw <- unlist(lapply(
    strsplit(commandArgs(TRUE), split = "=|\\s"),
    function(x) x[[1]]
  ))
  ...
  if (!all(args %in% expected)) {
    stop(
      sprintf(
        ...
        func_name,
        paste(paste0("--", args[!args %in% expected]), collapse = ",")
      ),
      call. = FALSE
    )
  }
...
check_required_args <- function(
  ...
  if (!all(required %in% args)) {
    stop(
      sprintf(
        "The following required arguments for %s are missing: %s",
        func_name,
  ...
  if (!is.null(required_one_of)) {
    if (sum(required_one_of %in% args) == 0) {
      stop(
        sprintf(
          "One of the following arguments should be specified: %s",
...
check_help <- function(args_raw) {
  if ("help" %in% args_raw && sum(rvat_cli_methods_flags %in% args_raw) == 1) {
    message(rvat_cli_help[[rvat_cli_methods_flags[
      rvat_cli_methods_flags %in% args_raw
    ]]])
    stopQuietly()
  } else if (
    "help" %in% args_raw && sum(rvat_cli_methods_flags %in% args_raw) != 1
  ) {
    message("")
    stopQuietly()
  }
