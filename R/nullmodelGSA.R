...
setMethod(
  ...
  definition = function(object, maxDist = 2.5e6) {
    ...
    check_missing <- !complete.cases(as.data.frame(object)[, c(
      "CHROM",
      "start",
      "end"
    )])
    if (sum(check_missing) > 0) {
      message(sprintf(
        "%s/%s records have missing data for `CHROM`, `start` and/or `end`. These are excluded.",
        sum(check_missing),
        nrow(object)
      ))
    ...
    for (chrom in chroms) {
      ...
      if (length(ranges_) == 1) {
        ...
        dist <- IRanges::distance(
          ranges_[1:(length(ranges_) - 1)],
          ranges_[2:length(ranges_)]
        )
...
setMethod(
  ...
  definition = function(
    ...
    ## Checks
    if (minR2 < 0 || minR2 > 1) {
      stop("`minR2` should be >=0 and <=1")
    }

    if (mean(object$unit %in% listUnits(aggdb)) < 1) {
      message(sprintf(
        "%s/%s units in the rvbResults object are not present in the aggdb, these will be excluded.",
    ...
    object <- addBlocks(object, maxDist = maxDist)
    ...
    for (i in sort(unique(object$block))) {
      if (verbose) {
        message(sprintf(
          "Analyzing block: %s/%s",
          i,
          length(unique(object$block))
        ))
      }
      if (i == load_chunk) {
        if (verbose) {
          message(sprintf("i = %s, loading new chunk", i))
        }
        blocks <- chunks[[chunk]]
      ...
      if (sum(object$block == i, na.rm = TRUE) == 1) {
        ...
        if (absolute) {
          cor_block <- abs(WGCNA::cor(
            t(mat[
              as.character(object$unit[object$block == i]),
              ,
              drop = FALSE
            ]),
            method = 'pearson'
...
setMethod(
  ...
  definition = function(
    ...
    if (!is.null(cormatrix)) {
      ...
      if (nonoverlap > 0) {
        message(sprintf(
          "%s/%s units are not present in the correlation matrix, these are excluded.",
          nonoverlap,
          nrow(object)
        ))
      ...
      if (nonoverlap > 0) {
        message(sprintf(
          "%s/%s units in the correlation matrix, are not present in the results, these are excluded",
          nonoverlap,
          nrow(object)
        ))
      }
      cormatrix <- cormatrix[
        as.character(object$unit),
        as.character(object$unit)
      ]
    }
    ...
    nullmodel <- GENESIS::fitNullModel(
