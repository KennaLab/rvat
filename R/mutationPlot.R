...
mutationPlot <- function(
  ...
  cds_remapped <- cds
  GenomicRanges::end(cds_remapped) <- GenomicRanges::end(cds_remapped) +
    mcols(cds_remapped)$gap
  GenomicRanges::start(cds_remapped) <- GenomicRanges::start(cds_remapped) +
    mcols(cds_remapped)$gap
  ...
  if (!is.null(customTracks)) {
    customTracks_remapped <- mapCDSGapsCustomTracks(
      customTracks,
      target_ranges = cds
    )
  ...
  if (!is.null(customTracks)) {
    if (!is.null(trackOrder)) {
      if (!"trackType" %in% colnames(customTracks_remapped)) {
        stop(
          "In order to set `trackOrder`, a field named `trackType` should be included in customTracks"
        )
      }
      factors <- list()
      for (track in trackOrder) {
        factors[[track]] <- customTracks_remapped[
          customTracks_remapped[["trackType"]] == track,
        ]$unit
      }
      ...
      if (splitByTrackType) {
        trackOrder <- c(
          "single variants",
          unique(customTracks_remapped$trackType)
        )
      } else {
  ...
  if (!is.null(customTracks)) {
    coords <- dplyr::tibble(
      ...
      ymin = cumsum(c(
        trackSpacing,
        rep(
          (trackSpacing + trackHeight),
          times = (dplyr::n_distinct(factors) - 1)
        )
      )),
      ymax = cumsum(c(
        (trackSpacing + trackHeight),
  ...
  if (!is.null(svHoverFields)) {
    if (is.null(names(svHoverFields))) {
      names(svHoverFields) <- svHoverFields
    }
  ...
  if (!is.null(customTracks)) {
    if (!is.null(trackHoverFields)) {
      if (is.null(names(trackHoverFields))) {
        names(trackHoverFields) <- trackHoverFields
      }
  ...
  if (!is.null(cdsHoverFields)) {
    if (is.null(names(cdsHoverFields))) {
      names(cdsHoverFields) <- cdsHoverFields
    }
  ...
  if (!is.null(customTracks) && splitByTrackType) {
    ...
    for (track in unique(customTracks_remapped$trackType)) {
      units <- unique(
        customTracks_remapped[customTracks_remapped$trackType == track, ]$unit
      )
    ...
    for (track in unique(customTracks_remapped$trackType)) {
      limits_list[[track]] <- c(
        (range_list[range_list$track == track, ]$min - padding),
    ...
    n <- padding *
      2 *
      dplyr::n_distinct(customTracks_remapped$trackType) +
      range_total
  ...
  if (!is.null(customTracks)) {
    ...
    if (splitByTrackType) {
      for (track in unique(customTracks_remapped$trackType)) {
        scales[[track]] <- as.formula(sprintf(
          'type == "%s" ~ ggplot2::scale_y_continuous(breaks=rowMeans(coords_list[["%s"]][,c("ymin", "ymax")]),labels=as.character(coords_list[["%s"]]$unit),limits=limits_list[["%s"]])',
          track,
          track,
          track,
          track
      ...
      scales[["tracks"]] <- type == "tracks" ~ ggplot2::scale_y_continuous(
        breaks = rowMeans(coords[, c("ymin", "ymax")]),
        labels = as.character(coords$unit)
      )
    }
    customTracks_remapped$type <- factor(
      customTracks_remapped$type,
      levels = trackOrder
    )
  }
  ...
  cdsplot <- ggplot2::ggplot(
    ...
    ### OR < 1
    {
      if (nrow(sv_negative) > 0) {
        ggplot2::geom_point(
          data = if (nrow(sv_negative) > 0) sv_negative else NULL,
          mapping = ggplot2::aes(
            x = POS,
            y = log10(P),
            fill = -log10(P),
    ...
    ### is.na(OR) (singletons)
    {
      if (nrow(sv_na) > 0) {
        ggplot2::geom_point(
          data = if (nrow(sv_na) > 0) sv_na else NULL,
          mapping = ggplot2::aes(
            x = POS,
            y = 0,
            text = text,
    ...
    {
      if (!is.null(impactScale)) {
        ggplot2::scale_shape_manual(values = impactScale)
      }
    } +
    ...
    {
      if (interactive) ggplot2::labs(fill = "", size = "", shape = "impact")
    } +
    ggh4x::force_panelsizes(rows = panelsizes)

  ## if interactive, use ggplotly to convert ggplot to plotly
  if (interactive) {
    return(plotly::ggplotly(cdsplot, tooltip = "text"))
  } else {
    cdsplot
  }
}
...
mapCDSGapsSV <- function(input, target_ranges) {
  ...
  mapped <- dplyr::tibble(
    ...
    POS = GenomicRanges::start(input_ranges[queryHits(overlaps)]) +
      mcols(target_ranges[subjectHits(overlaps)])$gap
  )
  input <- input %>%
    dplyr::select(-POS) %>%
...
mapCDSGapsCustomTracks <- function(input, target_ranges, warning = FALSE) {
  ...
  if (length(overlaps) < nrow(input)) {
    if (warning) {
      warning("Not all input ranges map to specified targets")
    }
  ...
  mapped_start <- dplyr::tibble(
    ...
    start = GenomicRanges::start(input_)[queryHits(overlaps)] +
      mcols(target_ranges[subjectHits(overlaps)])$gap
  )

  ## remap end coordinates
  input_ <- IRanges(
    start = GenomicRanges::end(input_ranges),
    end = GenomicRanges::end(input_ranges)
  )
  mcols(input_)$unit <- mcols(input_ranges)$unit
  mcols(input_)$i <- mcols(input_ranges)$i
  ...
  mapped_end <- dplyr::tibble(
    unit = mcols(input_[queryHits(overlaps)])$unit,
    i = mcols(input_[queryHits(overlaps)])$i,
    end = GenomicRanges::start(input_) +
      mcols(target_ranges[subjectHits(overlaps)])$gap
  )
