...
# additional checks
check_character_contains <- function(
  x,
  all_of = NULL,
  return_invalid = TRUE,
  ...,
  ...
  if (!is.null(all_of) && !all(x %in% all_of)) {
    if (return_invalid) {
      message <- sprintf(
        "`%s` contains invalid elements: %s",
        arg,
        paste0("'", paste(x[!x %in% all_of], collapse = ","), "'")
      )
    } else {
      message <- sprintf(
        "`%s` contains invalid elements.",
        arg
      )
    }
    rlang::abort(
...
check_length <- function(
  ...
  if (is.null(x) && !allow_null) {
    rlang::abort(
      sprintf("`%s` cannot be `NULL`", arg),
      arg = arg,
      call = call
    )
  }

  if (!is.null(x) && !is.null(equal) && length(x) != equal) {
    message <- sprintf(
      "`%s` should be of length %s",
      arg,
      equal
    )
    rlang::abort(
...
check_number_between <- function(
  ...
  if (including && !(x >= min && x <= max)) {
    message <- sprintf(
      "`%s` should be a value between %s and %s",
      arg,
      min,
      max
    )
...
# small wrapper for rlang's check functions
# set default of `allow_na` to `FALSE` and `call` to `NULL`
check_wrapper <- function(
  func,
  args,
  arg,
  length_equal = NULL,
  ...
  if (!is.null(length_equal) || !is.null(length_min) || !is.null(length_max)) {
    check_length(
      arg_value,
      arg = arg,
      equal = length_equal,
