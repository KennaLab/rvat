...
GSAresults_conditional <- geneSetAssoc(
  res,
  getGeneSet(genesetlist, as.character(topresults$geneSetName[1])),
  condition = getGeneSet(genesetlist, as.character(topresults$geneSetName[2:10])),
  covar = c("nvar"),
  test = c("lm"), 
  minSetSize = 10,
  maxSetSize = 500
...
aggregate(x = gdb, 
          ...
          maxMAF = 0.001,
          output = paste0(outdir, "/moderate.aggregate.aggdb"),
          verbose = FALSE
         )
```
...
Perform the gene set burden tests, for this we can use the `assocTest` method, with the first argument being an `aggdb` connection. Similar to a `varSetFile` or `geneSetFile` we can connect to the file containing aggregates (burden scores) using the `aggdb` method:

```{r}
aggfile <- aggdb(paste0(outdir, "/moderate.aggregate.aggdb"))
```

We can then run gene set burden tests using the `aggdb`, by passing it as the first argument to `assocTest` as shown below. Note that, in contrast to running `assocTest` directly on a genoMatrix or gdb, we cannot perform any variant filters when running `assocTest` on an `aggdb`. Any variant filters should be applied when generating the aggregates, as shown in step 1.
...
--aggdb $outdir/moderate.aggregate.aggdb \
--name maxMAF001 \
