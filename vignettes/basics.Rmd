...
# note that the filepath can also be provided directly without reading the file first.
# also note that `overWrite = TRUE` is specified here to overwrite the existing table.
uploadAnno(object = gdb, name = "varInfo", value = varinfopath, overWrite = TRUE)
```
...
# like for variant annotations, the filepath can also be provided directly
uploadCohort(object = gdb, name = "pheno", value = phenopath, overWrite = TRUE)
```
...
Variants to include can be specified in two main ways: 
1. by directly passing the VAR_ids to the `getGT()` method  
2. by providing a set of genomic ranges to retrieve  
3. by supplying a `varSet` (discussed later).
