...
test_that("reading and writing rvbResults works" ,{
  ...
  # read results w/o header
  resultfile_noheader <- tempfile()
  readr::write_tsv(as.data.frame(rvbresults_test), file = resultfile_noheader, progress = FALSE)
  expect_warning(rvbresults_read <- rvbResult(resultfile_noheader))
  ## should be identical except for metadata
  expect_identical(as.data.frame(rvbresults_test), as.data.frame(rvbresults_read), tolerance = 1e-10)
  ...
  # results w/o header
  resultfile_noheader <- tempfile()
  readr::write_tsv(as.data.frame(sv_results), file = resultfile_noheader, progress = FALSE)
  expect_warning(svresults_read <- singlevarResult(resultfile_noheader))

  ## should be identical except for metadata
  expect_identical(as.data.frame(sv_results), as.data.frame(svresults_read), tolerance = 1e-10)
